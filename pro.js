...
client.pointsMonitor = (dateformat, message) => {
  ...
  if (score.level < curLevel) {
        message.channel.send(`good luck <@!${message.author.id}> your level is ${curLevel} `);
...
message.channel.stopTyping(1)
 ...
 if(message.content.startsWith(prefix + 'stats')) {
    ...
    const embed = new Discord.RichEmbed()
      .setTitle('false!')
      .setColor('RED')
      .setDescription('**:alarm_clock: | you did it ! try again ' + moment().endOf('day').fromNow().replace('in ', 'بعد ') + '**')
...
var prefix =`!`;
  let command = message.content.split(" ")[0];
      if (command === prefix + "choose") {
        if(!args[0]) return message.reply('choose numberof your background')
        ...
        message.reply('succes');
            ...
            message.reply('you dont have this background')
        }
    }
...
.addField('Profile Old Tree','Price: $4000 Number: 5')
 .setImage("");
            ...
            if(wallpapers[args[0]].price > row.points) {
                message.reply('you cant buy it because you dont have money ')
            } else {
                if(dataPro[message.author.id].walls == wallpapers[args[0]]) return message.reply('you alredy have it !');
                else {
                    ...
                    message.reply('DONE!! You buy it .set '+args[0]);
                }
...
message.channel.stopTyping(1)
      ...
      if(message.content == `${prefix}level-up`) {
